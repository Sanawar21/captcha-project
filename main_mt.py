...
def main_loop(project_name, meta_data , bot, _context):
    global all_done_files
    global total_files
  # try:
    jwp = JsonWriter(project_name)
    jwp.file_name = jwp.META
    ...
    if completed:
      print(f'Crawler {bot["id"]} is completed.')
      return
    ...
    for file_num in to_do_files:
      ...
      if response == crawler.NO_RECORD:
        ...
        while not was_last:
          ...
          if demandante.endswith("DEMANDANTE"):
            demandante = demandante.replace("DEMANDANTE", "")
            # main_info.append("SWAPPED")
        ...
        crawler.home_page()
      all_done_files += 1
      jwp.change_field(jwp.COMPLETED_FILES, all_done_files)
      print(f"Crawler {bot['id']} files completed: {len(done_files)}/{len(bot_data[jw.FILES])}")
      print()
      print(f"Total files completed: {all_done_files}/{total_files}")
...
if __name__ == "__main__":
  ...
  if old_or_new[0] == "o":
    ...
    if len(incomplete) == 0:
      ...
      completed_files = meta_data[jw.COMPLETED_FILES]
  else:
    context = "new"
    ...
    meta_data = {
      ...
      jw.COMPLETED_FILES: completed_files,
    }
  ...
  all_done_files = completed_files
